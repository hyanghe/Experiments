...
class FNO2d(nn.Module):
    def __init__(self, modes1, modes2,  width, n_params=2):
        ...
        self.fc0 = nn.Linear(5, self.width) # input channel is 5: (a(x, y), x, y)
    ...
    def forward(self, x, top_htc, btm_htc):
    # def forward(self, x):
        # y = self.param_fc1(y)
        # y = F.relu(y)
        # y = y.view(-1, 1, 80, 80)
        ...
        # y = y.permute(0, 2, 3, 1)
        # print('x, top_htc, btm_htc: ', x.shape, top_htc.shape, btm_htc.shape)
        x = torch.cat((x, top_htc, btm_htc), dim=-1)
...
def get_dataLoader(batch_size, s):
    ...
    train_htc_top = np.load('./data/htc_top_train.npy')
    train_htc_btm = np.load('./data/htc_btm_train.npy')
    ...
    print('train_x max, min: ', x_train.max(), x_train.min())
    print('train_y max, min: ', y_train.max(), y_train.min())
    ...
    test_htc_top = np.load('./data/htc_top_test.npy')
    test_htc_btm = np.load('./data/htc_btm_test.npy')
    ...
    train_htc_top = torch.Tensor(train_htc_top)
    train_htc_btm = torch.Tensor(train_htc_btm)
    test_htc_top = torch.Tensor(test_htc_top)
    test_htc_btm = torch.Tensor(test_htc_btm)
    ...
    top_htc_normalizer = UnitGaussianNormalizer(train_htc_top)
    train_htc_top = top_htc_normalizer.encode(train_htc_top)
    test_htc_top = top_htc_normalizer.encode(test_htc_top)

    btm_htc_normalizer = UnitGaussianNormalizer(train_htc_btm)
    train_htc_btm = btm_htc_normalizer.encode(train_htc_btm)
    test_htc_btm = btm_htc_normalizer.encode(test_htc_btm)
    ...
    print('x_test_shape: ', x_test.shape)
    print('y_test_shape: ', y_test.shape)
    print('train_htc_top shape: ', train_htc_top.shape)
    print('test_htc_top shape: ', test_htc_top.shape)
    print('train_htc_btm shape: ', train_htc_btm.shape)
    print('train_htc_btm shape: ', train_htc_btm.shape)
    # raise
    x_train = x_train.reshape(-1,s,s,1)
    x_test = x_test.reshape(-1,s,s,1)
    train_htc_top = train_htc_top.reshape(-1,s,s,1)
    test_htc_top = test_htc_top.reshape(-1,s,s,1)
    train_htc_btm = train_htc_btm.reshape(-1,s,s,1)
    test_htc_btm = test_htc_btm.reshape(-1,s,s,1)
    ...
    print('test_htc_top shape: ', test_htc_top.shape)
    print('train_htc_btm shape: ', train_htc_btm.shape)
    print('test_htc_btm shape: ', test_htc_btm.shape)
    # raise

    # 2. Split into train / validation partitions
    dataset = torch.utils.data.TensorDataset(x_train, y_train, train_htc_top, train_htc_btm)
    test_dataset = torch.utils.data.TensorDataset(x_test, y_test, test_htc_top, test_htc_btm)
    val_percent = 0.1
    ...
    return trainloader, valloader, testloader, x_normalizer, y_normalizer, x_test, y_test, test_htc_top, test_htc_btm, n_train, n_val, n_test
...
def test_net(model,
    ...
    with torch.no_grad():
        ...
        for x, y, top_htc, btm_htc in test_loader:
            x, y, top_htc, btm_htc = x.cuda(DEVICE_NUM), y.cuda(DEVICE_NUM), top_htc.cuda(DEVICE_NUM),\
                                    btm_htc.cuda(DEVICE_NUM)
            # print('x, y, top_htc, btm_htc: ', x.shape, y.shape, top_htc.shape, btm_htc.shape)
            # print('model(x, top_htc, btm_htc): ', model(x, top_htc, btm_htc).shape)
            # raise
            out = model(x, top_htc, btm_htc).reshape(-1, s, s)
...
if __name__ == '__main__':
    ...
    train_loader, val_loader, test_loader, x_normalizer, y_normalizer, x_test, y_test, test_htc_top, test_htc_btm, ntrain, nval, ntest  = get_dataLoader(batch_size, s)
    ...
    if mode == 'train':
        ...
        for ep in range(epochs):
            ...
            t1 = default_timer()
            train_l2 = 0
            for x, y, top_htc, btm_htc in train_loader:
                x, y, top_htc, btm_htc = x.cuda(DEVICE_NUM), y.cuda(DEVICE_NUM), top_htc.cuda(DEVICE_NUM),\
                                        btm_htc.cuda(DEVICE_NUM)
            ...
            val_l2 = 0.0
            with torch.no_grad():
                for x, y, top_htc, btm_htc in val_loader:
                    x, y, top_htc, btm_htc = x.cuda(DEVICE_NUM), y.cuda(DEVICE_NUM), top_htc.cuda(DEVICE_NUM),\
                                            btm_htc.cuda(DEVICE_NUM)
            ...
            with torch.no_grad():
                if val_l2 < min_test_l2:
                    ...
                    x, y, htc_top_v, htc_btm_v = x_test[idx:idx+1].cuda(DEVICE_NUM), y_test[idx:idx+1].cuda(DEVICE_NUM),\
                     test_htc_top[idx:idx+1].cuda(DEVICE_NUM), test_htc_btm[idx:idx+1].cuda(DEVICE_NUM)
                    # print('model(x, param) shape: ', model(x, param).shape)
                    # raise
                    # print('x,y,param: ', x.shape, y.shape, param.shape)
                    # raise
                    pred = model(x, htc_top_v, htc_btm_v).reshape(1, s, s)
