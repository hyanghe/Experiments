...
class deepONet(nn.Module):
    def __init__(self, in_channels=1, mid_channels=32, in_features=2, mid_features=32, activation=nn.ReLU()):
        ...
        self.trunk = nn.Sequential(
            nn.Conv2d(in_channels=2, out_channels = mid_channels, kernel_size = 3, padding=1),
            nn.Conv2d(in_channels=mid_channels, out_channels=mid_channels, kernel_size=3, padding=1),
            # nn.BatchNorm2d(mid_channels),
            activation,
            nn.Dropout(self.dropout),
        ...
        self.merge = nn.Sequential(
            # nn.Conv2d(in_channels=mid_channels, out_channels=64, kernel_size=3, padding='same'),
            nn.Conv2d(in_channels=mid_channels, out_channels=256, kernel_size=3, padding=1),
        )
    ...
    def forward(self, x1, x2):
        ...
        x2 = self.trunk(x2)
        out = torch.mul(x1, x2)
        # print('Before merge, after mul: ', out.shape)
        out = self.merge(out)
...
def get_data():
    ...
    train_htc = torch.cat((train_htc_top, train_htc_btm), dim=-1)
    test_htc = torch.cat((test_htc_top, test_htc_btm), dim=-1)
    ...
    train_htc = train_htc.permute((0,3,1,2))
    ...
    test_htc = test_htc.permute((0,3,1,2))

    print('train_x, train_y: ', train_x.shape, train_y.shape)
    # raise
    dataset = torch.utils.data.TensorDataset(train_x, train_y, train_htc)
    test_dataset = torch.utils.data.TensorDataset(test_x, test_y, test_htc)
    val_percent = 0.1
...
def validate(net, dataloader, device, mx, mn):
    ...
    for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
        image, mask_true, htc = batch[0], batch[1], batch[2]
        image = image.to(device=device, dtype=torch.float32)
        mask_true = mask_true.to(device=device, dtype=torch.float32)
        htc = htc.to(device=device, dtype=torch.float32)
        
        with torch.no_grad():
            mask_pred = net(image, htc)
...
def test(net, testloader, device, T_min, T_max):
    ...
    with torch.no_grad():
        for batch in tqdm(testloader, desc='Testing round', unit='batch', leave=False):
            inputs_image, targets, htc = batch[0], batch[1], batch[2]
            inputs_image = inputs_image.to(device=device, dtype=torch.float32)
            targets = targets.to(device=device, dtype=torch.float32)
            htc = htc.to(device=device, dtype=torch.float32)

            outputs = net(inputs_image, htc)
    ...
    if mode == 'train':
        ...
        for epoch in range(Epochs+1):
            ...
            for batch in tqdm(trainloader):
                power, T, htc = batch[0], batch[1], batch[2]
