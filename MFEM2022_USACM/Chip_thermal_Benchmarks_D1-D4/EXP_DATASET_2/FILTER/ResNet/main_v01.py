...
def train(net, epoch, trainloader, valloader, optimizer):
    ...
    # correct = 0
    # total = 0
    for batch_idx, (inputs, targets, params) in tqdm(enumerate(trainloader)):
    ...
    for batch_idx_val, (inputs_val, targets_val, params_val) in tqdm(enumerate(valloader)):
    # for batch in trainloader:
        inputs_val, targets_val, params_val = inputs_val.to(device), targets_val.to(device), params_val.to(device)
        optimizer.zero_grad()
        outputs_val = net(inputs_val, params_val)
...
def test(net, testloader, T_min, T_max):
    ...
    pred = []
    with torch.no_grad():
        for batch_idx, (inputs, targets, params) in enumerate(testloader):
            inputs, targets, params = inputs.to(device), targets.to(device), params.to(device)
            outputs = net(inputs, params)
            loss = criterion(outputs, targets)
